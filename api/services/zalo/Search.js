...
module.exports = async function(phone,{cookie,secretKey,imei}){
        ...
        if(check){
            ...
            let j = JSON.parse(t);
            if(j.error_code !== 0) throw 'Tra cứu zalo bị chặn';
            if(j.data){
                return j.data.uid.toString();
            }else{
                return null;
            }
        }else{
            throw 'Có lỗi xảy ra khi lấy ID người dùng';
       }
   };
